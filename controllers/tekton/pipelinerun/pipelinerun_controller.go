...
func (r *PipelineRunReconciler) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, pipelineRun); err != nil {
		r.Log.Error(err, "unable to fetch pipeline crd")
		return ctrl.Result{}, client.IgnoreNotFound(err)
	}

	if err := r.reconcileTektonCrd(ctx, pipelineRun); err != nil {
		r.Log.Error(err, "Failed to reconcile Tekton PipelineRun resources.")
...
func (r *PipelineRunReconciler) reconcileTektonCrd(ctx context.Context, pipelineRun *devopsv2alpha1.PipelineRun) error {
	if err := r.reconcileTektonPipelineRun(ctx, &pipelineRun.Spec); err != nil {
		return err
	}
	return nil
}
...
func (r *PipelineRunReconciler) reconcileTektonPipelineRun(ctx context.Context, pipelineRun *devopsv2alpha1.PipelineRunSpec) error {

	r.Log.Info(fmt.Sprintf("Going to create Tekton PipelineRun resource called %s", pipelineRun.Name))

	// translate PipelineRun to Tekton PipelineRun
	tPipelineRun := &tektonv1.PipelineRun{}
	if err := r.Get(ctx, types.NamespacedName{Namespace: "default", Name: pipelineRun.Name}, tPipelineRun); err != nil {
		...
		tektonPipelineRun := &tektonv1.PipelineRun{
			...
			ObjectMeta: metav1.ObjectMeta{Name: pipelineRun.Name, Namespace: "default"},
		...
		r.Log.Info(fmt.Sprintf("Tekton PipelineRun resource %s created successfully", pipelineRun.Name))
	} else {
		// This means that a Tekton PipelineRun resource has already exists in the given namespace,
		// which can be a problem.
		r.Log.Info(fmt.Sprintf("Tekton PipelineRun resource %s already exists!", pipelineRun.Name))
	}
